...
pub trait TrieKey: Hash + Index<usize> + Index<Range<usize>, Output=Self> + PartialEq + ToOwned
where <Self as Index<usize>>::Output: Clone + PartialEq + PartialOrd,
      <Self as ToOwned>::Owned: Clone + Hash + PartialEq
{
    fn len(&self) -> usize;
}
...
where K: TrieKey + Hash + Index<usize> + Index<Range<usize>, Output=K> + PartialEq + ToOwned + ?Sized,
      T: TrieNode<K, V>,
      V: Clone,
      <K as Index<usize>>::Output: Clone + Debug + PartialEq + PartialOrd,
      <K as ToOwned>::Owned: Clone + Debug + Hash + PartialEq {
    ...
    Pure(ArrayHash<twox_hash::XxHash64, <K as ToOwned>::Owned, V>),
    /// Hybrid contaainer type. It can have more than one parent trie node.
    Hybrid((ArrayHash<twox_hash::XxHash64, <K as ToOwned>::Owned, V>, core::ops::RangeInclusive<<K as Index<usize>>::Output>))
}
...
where K: TrieKey + Hash + Index<usize> + Index<Range<usize>, Output=K> + PartialEq + ToOwned + ?Sized,
    ...
    fn new_split(bucket: ArrayHash<twox_hash::XxHash64, <K as ToOwned>::Owned, V>, threshold: usize) -> Self;
    /// Get a child node of given key from current node.
    fn child(&'_ self, key: &<K as Index<usize>>::Output) -> &'_ NodeType<K, Self, V>;
            ...
            if offset < key.len() {
                ...
                match node {
                    ...
                    NodeType::Pure(child) | NodeType::Hybrid((child, _)) => {
                        ...
                        return child.coerce_get(&key[offset..key.len()])
                    }
                }
            } else {
                return parent.value()
    ...
    fn prefix<'a, 'b>(&'a self, key: &'b K) -> PrefixIterator<'a, 'b, K, Self, V> {
        PrefixIterator {
            ...
            query: &key[0..core::cmp::min(self.max_key_len(), key.len())] 
        }
    }
...
/// A trait that all item going to be return by [StartsWith](trait.StartsWith.html)
/// need to implement.
pub trait StartsWithItem<K, V> 
...
/// A trait that if implemented on any type, mean it can be used to lookup
/// for a key/value which has some specific prefix in it key
pub trait StartsWith<'a, K, V> 
where K: 'a + Copy + core::hash::Hash + PartialEq + PartialOrd + Sized, 
      V: 'a + Clone {
    /// An Iterator that return key and value of the trie which prefixed by some value.
    type Iter:'a + Iterator<Item=Self::IterItem>;
    /// An Item that iterator should yield on each iteration.
    type IterItem: StartsWithItem<K, V>;

    /// Find all possible key in trie that starts with given key.
    /// 
    /// # Parameter
    /// `key` - a slice of key expects to be prefix of some keys in this trie.
    /// 
    /// # Return
    /// It return iterator that shall yield both key and value of this trie that has
    /// given key as prefix of the key. However, the choice of how to return is up
    /// to implementer.
    fn starts_with(&'a self, key: &'a [K]) -> Self::Iter;
}
...
where K: TrieKey + Hash + Index<usize> + Index<Range<usize>, Output=K> + PartialEq + ToOwned + ?Sized,
    ...
    bucket: Option<&'a ArrayHash<twox_hash::XxHash64, <K as ToOwned>::Owned, V>>,
...
impl<'a, 'b: 'a, K, T, V>  Iterator for PrefixIterator<'a, 'b, K, T, V> 
where K: TrieKey + Hash + Index<usize> + Index<Range<usize>, Output=K> + PartialEq + ToOwned + ?Sized,
      T: 'a + TrieNode<K, V>,
      V: Clone,
      <K as Index<usize>>::Output: Clone + Debug + PartialEq + PartialOrd,
            ...
            while self.bucket.is_none() {
                match self.node.child(&self.query[self.cursor]) {
                    ...
                    NodeType::Trie(ref t) => {
                        ...
                        if t.value().is_some() {
                            return Some((&self.query[0..(self.cursor + 1)], t.value().as_ref().unwrap()))
                        }
                    },
...
impl<'a, 'b: 'a, K, T, V> core::iter::FusedIterator for PrefixIterator<'a, 'b, K, T, V> 
where K: TrieKey + Hash + Index<usize> + Index<Range<usize>, Output=K> + ToOwned,
      T: 'a + TrieNode<K, V>,
      V: Clone,
      <K as Index<usize>>::Output: Clone + Debug + PartialEq + PartialOrd,
      <K as ToOwned>::Owned: Clone + Debug + Hash + PartialEq {
